...
if [ -d "$JAVA_25" ]; then
    ...
    JAVA_HOME=$JAVA_25 ./gradlew clean build -PmcVersion=26.1 --no-daemon -q && echo "=== 26.1 OK ===" || echo "=== 26.1 SKIPPED (Loom Java 25 unsupported) ==="
fi

echo "=== All builds done ==="
ls -lh build/libs/autojoin-*.jar 2>/dev/null
